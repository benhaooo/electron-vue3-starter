<template>
  <div id="app" class="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
    ...
    <div v-if="showTitleBar" class="title-bar bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 h-8 flex items-center justify-between px-4">
      <div class="flex items-center space-x-2">
        <div class="w-3 h-3 rounded-full bg-red-500"></div>
        <div class="w-3 h-3 rounded-full bg-yellow-500"></div>
        <div class="w-3 h-3 rounded-full bg-green-500"></div>
      </div>
      <div class="title-bar-text text-sm font-medium text-gray-600 dark:text-gray-300">{{ appTitle }}</div>
      <div class="w-16"></div>
    </div>
        ...
        <div class="flex justify-between h-16">
          <div class="flex items-center space-x-8">
            <div class="flex-shrink-0 flex items-center">
              <h1 class="nav-title text-xl font-bold text-primary-600 dark:text-primary-400">Electron Vue3 Starter</h1>
            </div>
